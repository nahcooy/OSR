...
# 🔹 CoAtNet 기반 모델 정의
class CoAtNetForClassification(nn.Module):
    def __init__(self, num_classes=7, pretrained=True):
        super(CoAtNetForClassification, self).__init__()
        self.model = timm.create_model('coatnet_2_224', pretrained=pretrained, num_classes=num_classes)
...
# 🔹 모델 로드
def load_model():
    model = CoAtNetForClassification(num_classes=args['num_classes'], pretrained=args['pretrained'])
    return model
