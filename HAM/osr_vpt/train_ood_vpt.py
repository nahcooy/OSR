# 파일 이름: /nahcooy/OSR/HAM/osr_vit/train_ood_vpt.py
import torch
...
from model import VisionTransformer, VPT_OODTransformer, load_checkpoint
...
def inference_ood(config, device, device_ids, classes_mean):
    ...
    checkpoint_path = os.path.join(config.checkpoint_dir, 'vpt_ood_best.pt')
...
def train_ood(config, device, device_ids):
    ...
    if os.path.exists(vit_checkpoint_path):
        ...
        vit_model.load_state_dict(checkpoint['model_state_dict'], strict=False)  # strict=False 추가
    ...
    ood_checkpoint_path = os.path.join(config.checkpoint_dir, 'vpt_ood_best.pt')
    if os.path.exists(ood_checkpoint_path):
        ...
        optimizer = torch.optim.AdamW(
            params=list(ood_model.vit.transformer.prompts.parameters()) + list(ood_model.head.parameters()),
            lr=config.lr,
...
if __name__ == "__main__":
    class Config:
        ...
        warmup_steps = 500
        random_seed = 42
        checkpoint_dir = "/nahcooy/OSR/HAM/osr_vpt/checkpoints/vpt"
