...
from model import VisionTransformer, OODTransformer, load_checkpoint
...
def valid_ood_epoch(epoch, vit_model, ood_model, val_loader, classes_mean, device):
    ...
    # Known 데이터만 사용하므로 binary_targets는 없음, 대신 클래스별 거리 기반 메트릭 계산
    distances = np.array([np.min([np.linalg.norm(pred - mean) for mean in classes_mean.cpu().numpy()]) for pred in all_preds])
    
    # Known 데이터로만 평가하므로 AUROC는 계산 불가, 대신 평균 거리와 분산 출력
    avg_distance = np.mean(distances)
    std_distance = np.std(distances)
    
    print(f"[{datetime.now()}] Epoch {epoch} Validation (Known Only):")
    print(f"Average Distance: {avg_distance:.4f}, Std Distance: {std_distance:.4f}")
    return {'avg_distance': avg_distance, 'std_distance': std_distance}

def inference_ood(config, device, device_ids, classes_mean):
    print(f"[{datetime.now()}] Starting OOD Inference on Unknown Data")
    ...
    if os.path.exists(checkpoint_path):
        ...
        print(f"[{datetime.now()}] Loaded OOD model from {checkpoint_path} with AUROC: {checkpoint['auroc']:.4f}")
    else:
    ...
    val_unknown_dataloader = DataLoader(val_unknown_dataset, batch_size=config.batch_size, shuffle=False, num_workers=config.num_workers)
    
    # 추론 수행
    ood_model.eval()
    all_preds, all_targets = [], []
    with torch.no_grad():
        for batch_data, batch_target in val_unknown_dataloader:
            batch_data, batch_target = batch_data.to(device), batch_target.to(device)
            batch_pred = ood_model(batch_data)
            all_preds.append(batch_pred.cpu().numpy())
            all_targets.append(batch_target.cpu().numpy())
    ...
    print(f"[{datetime.now()}] OOD Inference Results (Unknown Only):")
...
def train_ood(config, device, device_ids):
    ...
    checkpoint_path = os.path.join(config.checkpoint_dir, 'vit_best.pt')
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location=device)
        vit_model.load_state_dict(checkpoint['model_state_dict'])
        print(f"[{datetime.now()}] Loaded ViT model from {checkpoint_path} with AUROC: {checkpoint['auroc']:.4f}")
    else:
        raise FileNotFoundError(f"Checkpoint {checkpoint_path} not found. Please run Stage 1 first.")
    ...
    val_known_dataloader = DataLoader(val_known_dataset, batch_size=config.batch_size, shuffle=False, num_workers=config.num_workers)  # Known만 사용
    ...
    best_avg_distance = float('inf')  # Known 데이터의 평균 거리를 최소화하는 방향으로
    for epoch in range(1, config.epochs + 1):
        train_loss = train_ood_epoch(epoch, vit_model, ood_model, train_dataloader, criterion, optimizer, classes_mean, device)
        val_metrics = valid_ood_epoch(epoch, vit_model, ood_model, val_known_dataloader, classes_mean, device)
        
        # 최고 모델 저장 (평균 거리가 최소일 때)
        if val_metrics['avg_distance'] < best_avg_distance:
            ...
            }, os.path.join(config.checkpoint_dir, 'ood_best.pt'))
...
if __name__ == "__main__":
    ...
    # Stage 1: ViT 훈련
    print(f"[{datetime.now()}] stage 1 시작")
    vit_model = train_vit(config, device, device_ids)
    
    print()
    print()    
    print(f"[{datetime.now()}] stage 1 끝")
    print(f"[{datetime.now()}] stage 2 시작")
    print()
    print()

    # Stage 2: OOD 훈련 (vit_best.pt 로드)
    classes_mean = train_ood(config, device, device_ids)
    
    # Stage 2 이후: Unknown 데이터에 대한 추론
