...
from utils import TensorboardWriter, MetricTracker, accuracy, write_json  # .utils -> osr_vit.utils
...
def run_model(model, loader, device):
    ...
    tgt_list = []
    with torch.no_grad():
        for images, target in loader:  # tqdm 제거
...
def train_epoch(epoch, model, data_loader, criterion, optimizer, lr_scheduler, metrics, config, device):
    ...
    metrics.reset()
    running_loss = 0.0
    for batch_idx, (batch_data, batch_target) in enumerate(data_loader):  # tqdm 제거
        ...
        if batch_idx % 50 == 49:  # 50번째 배치마다 출력
...
def valid_epoch(epoch, vit_model, ood_model, val_known_loader, full_loader, criterion, metrics, config, classes_mean, device):
    ...
    with torch.no_grad():
        for batch_data, batch_target in val_known_loader:
            batch_data, batch_target = batch_data.to(device), batch_target.to(device)
            batch_pred = vit_model(batch_data)
            loss = criterion(batch_pred, batch_target)
            ...
            all_targets.append(batch_target.cpu().numpy())  # 리스트에 추가
    known_loss = np.mean(losses)
    known_acc1 = np.mean(acc1s)
    known_acc5 = np.mean(acc5s)
    all_preds = np.concatenate(all_preds)  # 그대로 유지
    all_targets = np.concatenate(all_targets)  # np.array 대신 np.concatenate로 균일화
    ...
    with torch.no_grad():
        for batch_data, batch_target in full_loader:
            batch_data, batch_target = batch_data.to(device), batch_target.to(device)
    ...
    all_targets = np.concatenate(all_targets)  # np.array 대신 np.concatenate
    binary_targets = (all_targets == 6).astype(int)


    print(f"all_preds shape: {all_preds.shape}")
    print(f"classes_mean shape: {classes_mean.shape}")
    for pred in all_preds[:5]:  # 첫 5개 샘플만 확인
        print(f"pred shape: {pred.shape}")
    for mean in classes_mean.cpu().numpy()[:5]:
...
def train(config, device, device_ids):
    ...
    print(f"[{datetime.now()}] Step {step}-1: Loading train dataset")
    ...
    print(f"[{datetime.now()}] Step {step}-2: Loading val_known dataset")
    ...
    print(f"[{datetime.now()}] Step {step}-3: Loading val_full dataset")
    ...
    train_emb, train_targets = run_model(ood_model, train_dataloader, device)  # vit_model 대신 ood_model 사용
    ...
    print(f"Number of classes in train: {len(in_classes)}, Classes: {in_classes}")
    print(f"classes_mean shape: {classes_mean.shape}")
    step += 1
    ...
    total_steps = config.epochs * len(train_dataloader)  # 총 스텝 수 계산
    lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer=optimizer,
        max_lr=config.lr,
        pct_start=config.warmup_steps / total_steps,  # warmup 비율 조정
        total_steps=total_steps
    )

    step += 1
    print(f"[{datetime.now()}] Step {step}: Starting training")
    print(f"Length of train loader: {len(train_dataloader)}, total epochs: {config.epochs}, total steps: {total_steps}")
    for epoch in range(1, config.epochs + 1):
        ...
        save_best_models(config.checkpoint_dir, epoch, vit_model, optimizer, lr_scheduler, device_ids, valid_metrics)
