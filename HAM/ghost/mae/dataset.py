...
def getHAM10000Dataset(data_path='./data', **args):
    ...
    train_meta, val_meta = train_test_split(close_metadata, test_size=0.2, random_state=args.get('random_seed', 42))

    if split == 'train':
        filtered_metadata = train_meta  # Known 80%
        mapping = close_mapping
    elif split == 'val_known':
        filtered_metadata = val_meta  # Known 20%
        mapping = close_mapping
    ...
    elif split == 'total_known':
        filtered_metadata = close_metadata  # Known 전체 사용
        mapping = close_mapping
    else:
