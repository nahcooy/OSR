...
def evaluate_metrics(true_labels, predictions, joint_energies, all_labels, known_labels, unknown_labels, tau_nor=1.0, tau_unk=3.0, category_name=""):
    ...
    for tau_version in tau_versions:
        ...
        # Joint Energies 분포 출력
        print(f"Joint Energies - Min: {np.min(joint_energies):.4f}, Max: {np.max(joint_energies):.4f}, Mean: {np.mean(joint_energies):.4f}")
...
def main(epoch=10, tau_nor=1.0, tau_unk=3.0, lambda_kd=1.0, lambda_ent=0.1, save_path_prefix=""):
    ...
    classifier = Classifier(classid_list=known_labels).to("cuda:1")
    copycat = Copycat(num_classes=len(known_labels)).to("cuda:1")
    ...
    for current_epoch in range(epoch):
        ...
        print(f"Epoch {current_epoch+1}/{epoch} - Training started")
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 50 == 0:
                print(f"Batch {batch_idx+1}: Copycat Loss = {copycat_loss.item():.4f}, Classifier Loss = {classifier_loss.item():.4f}")
        ...
        for category, loader in val_loaders.items():
            print(f"Validation for {category} started")
            ...
            print(f"Validation for {category} ended")
        ...
        if val_loss_avg_total < best_val_loss:
            ...
            print(f"Best model saved at epoch {best_epoch+1} with Val Loss: {best_val_loss:.4f}")

    print(f"Training completed. Best model was at epoch {best_epoch+1}")

if __name__ == "__main__":
    main(epoch=200, tau_nor=1.0, tau_unk=3.0, lambda_kd=1.0, lambda_ent=0.1, save_path_prefix="./checkpoints/train_reverse_ver2_0402")