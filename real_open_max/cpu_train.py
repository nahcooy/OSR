...
class OpenMax:
    ...
    def fit_weibull(self, train_loader):
        ...
        with torch.no_grad():
            for images, targets in train_loader:
                outputs = self.model(images)  # CPU에서 실행
    ...
    def openmax_recalibrate(self, logits):
        scores = torch.sigmoid(logits).numpy()  # CPU에서 실행
        batch_size = scores.shape[0]
        openmax_scores = np.zeros(batch_size)
        recalibrated_scores = np.zeros_like(scores)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for images, targets in val_loader:
                ...
                all_outputs.append(logits.numpy())
                all_targets.append(targets.numpy())
        ...
        if np.sum(known_mask) > 0:
            ...
            val_loss_multiclass = criterion(
                torch.tensor(all_openmax_preds[known_mask]),
                torch.tensor(known_targets)
            ).item()
        else:
        ...
        val_loss_unknown = criterion(
            torch.tensor(closed_outputs),
            torch.tensor(closed_targets)
        ).item()
...
def main():
    ...
    train_loader, val_loader = load_data(
        ...
        batch_size=16, train_data_ratio=0.3, val_data_ratio=1.0
    )
